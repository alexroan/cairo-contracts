...
namespace IERC677Receiver:
    func onTokenTransfer(sender: felt, value: Uint256, data: felt):
    end
end
...
@external
func transferAndCall{
    syscall_ptr : felt*, 
        pedersen_ptr : HashBuiltin*,
        range_check_ptr
    }(recipient: felt, amount: Uint256, data: felt) -> (success: felt):
    let (sender) = get_caller_address()
    _transferAndCall(sender, recipient, amount, data)
...
func _transferAndCall{
        syscall_ptr : felt*, 
        pedersen_ptr : HashBuiltin*,
        range_check_ptr
    }(sender: felt, recipient: felt, amount: Uint256, data: felt):
    _transfer(sender, recipient, amount)
    IERC677Receiver.onTokenTransfer(contract_address=recipient, sender=sender, value=amount, data=data)
    return ()
end
